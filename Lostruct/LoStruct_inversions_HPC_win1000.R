...
## Kaichi Huang 2019 May with very minor updates by Andre Moncrieff
...
window_size <- 1000 # window size = 1000 SNPs
...
message("k_kept = ", k_kept)
message("mc.cores = 1")
message("Input BCF = lostruct.bcf")

# LoStruct
# LoStruct
message("Starting LoStruct pipeline: ", Sys.time())

bcf.file <- "lostruct.bcf"

message("Starting vcf_positions: ", Sys.time())
sites <- vcf_positions(bcf.file)
saveRDS(sites, file="checkpoint.sites.win1000.rds")
message("Finished vcf_positions: ", Sys.time())

message("Starting vcf_windower: ", Sys.time())
win.fn.snp <- vcf_windower(bcf.file, size=window_size, type="snp", sites=sites)
saveRDS(region(win.fn.snp)(), file="checkpoint.raw_window_regions.win1000.rds")
message("Finished vcf_windower: ", Sys.time())

message("Starting eigen_windows: ", Sys.time())
snp.pca <- eigen_windows(win.fn.snp, k=2, mc.cores=1)
saveRDS(snp.pca, file="checkpoint.snp.pca.win1000.rds")
message("Finished eigen_windows: ", Sys.time())

message("Starting pc_dist: ", Sys.time())
pcdist <- pc_dist(snp.pca, mc.cores=1)
saveRDS(pcdist, file="checkpoint.pcdist.raw.win1000.rds")
message("Finished pc_dist: ", Sys.time())

message("Filtering NA/NaN windows: ", Sys.time())
na.wins <- is.na(snp.pca[,1])
pcdist <- pcdist[!na.wins, !na.wins]
nan.wins <- pcdist[,1] == "NaN"
pcdist <- pcdist[!nan.wins, !nan.wins]
saveRDS(pcdist, file="checkpoint.pcdist.filtered.win1000.rds")
message("Finished filtering distance matrix: ", Sys.time())

message("Starting cmdscale/MDS: ", Sys.time())
mds <- cmdscale(pcdist, eig=TRUE, k=k_kept)
saveRDS(mds, file="checkpoint.mds.win1000.rds")
message("Finished cmdscale/MDS: ", Sys.time())

message("Building win.regions table: ", Sys.time())
...
saveRDS(win.regions, file="allele_stats.lostruct.win1000.windows.rds")
message("Finished win.regions table: ", Sys.time())
message("Number of retained windows: ", nrow(win.regions))

# Look for "chromosomally clustered" windows along each MDS coordinate (in both positive and negative directions)
...
pdf("allele_stats.lostruct.win1000.pdf", height=6, width=14)
for (i in 1:nrow(sig_mds_clusters)){
  coord <- pull(sig_mds_clusters[i,1])
  direction <- pull(sig_mds_clusters[i,2])
...
dev.off()
write_tsv(cluster_genotypes, "cluster_genotypes.win1000.tsv")
write_tsv(mds_info, "mds_info.win1000.tsv")