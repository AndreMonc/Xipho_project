...
infile_raisd_belem <- "combined_RAiSD_output.sorted.cleaned.belem.bed"
infile_raisd_xingu <- "combined_RAiSD_output.sorted.cleaned.xingu.bed"
...
## If your master already has Xingu_RTH_inverse / Belem_RTH_inverse / Tapajos_Xingu_RTH_inverse, it uses them;
...
dat2 <- get_or_inverse(dat2, "Tapajos_Xingu_RTH_inverse", fallback_raw = "Tapajos_Xingu_RTH", new_name = "Tapajos_Xingu_RTH_inverse")
...
panel_labels <- c(
  ...
  Tapajos_Xingu_RTH_inverse = "Tapajos-Xingu RTH (inverse)",
  ...
  `U statistic Xingu (RAISD)` = "Xingu RAiSD u",
  `U statistic Belem (RAISD)` = "Belem RAiSD u"
)
...
save_peak_panel <- function(peak_row) {
  ...
  region_master <- dat2 %>%
    ...
    transmute(
      ...
      `Tapajos_Xingu_RTH_inverse` = as.numeric(Tapajos_Xingu_RTH_inverse),
