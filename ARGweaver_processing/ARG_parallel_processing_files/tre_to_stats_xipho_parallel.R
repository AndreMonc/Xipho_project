...
if (length(args) < 2) {
  stop("Usage: Rscript tre_to_stats_xipho_parallel.R <treeFile> <statsDir>")
}
...
belem_n   <- as.numeric(pop_counts[bel_pop])
...
treeStats <- c(
  ...
  ### Xingu-Belem half-half clade statistics.
  "Xin_Bel_TMRCAH",
  ...
  ### Values >1 mean the Xingu-Belem half-half clade is older than Tapajos-Xingu.
  "Tap_Xin_JCR",

  ### Per-population RTH:
  ###   youngest clade containing >= half of that population / TMRCAH_all.
  paste(popNames, "RTH", sep = "_"),

  ### Per-population enrichment:
  ###   maximum enrichment of that population in any subtree.
  paste(popNames, "enrich", sep = "_")
)
...
### ">= half of population A and >= half of population B" rule.
###
### We choose:
...
for (coord in coordinates) {
  ...
  ### Counts across the full tree, used as denominators for enrichment.
  ...
  ### Add enrichment columns for each population.
  subtree_stats_df <- computePopEnrichment(
    subtree_stats_df,
    popNames,
    totalTreeCounts
  )
  ...
  if (!is.null(tap_xin_clade)) {
    ...
    tree_stats_df$Tap_Xin_RTH_original <- round(
      tap_xin_clade$age / tree_stats_df$TMRCA_all,
      digits = 4
    )

    tree_stats_df$Tap_Xin_RTH_prime <- round(
      tap_xin_clade$age / tree_stats_df$TMRCAH_all,
      digits = 4
    )
  }

  ### -----------------------------
  ### 8b. Xingu-Belem half-half clade
  ### -----------------------------

  xin_bel_df <- subtree_stats_df[
    subtree_stats_df[[xin_count_col]] >= xingu_n / 2 &
      subtree_stats_df[[bel_count_col]] >= belem_n / 2,
  ]

  xin_bel_clade <- choose_youngest_clade(
    xin_bel_df,
    "xin_bel_joint_count"
  )

  if (!is.null(xin_bel_clade)) {
    tree_stats_df$Xin_Bel_TMRCAH <- xin_bel_clade$age

    tree_stats_df$Xin_Bel_RTH_original <- round(
      xin_bel_clade$age / tree_stats_df$TMRCA_all,
      digits = 4
    )

    tree_stats_df$Xin_Bel_RTH_prime <- round(
      xin_bel_clade$age / tree_stats_df$TMRCAH_all,
      digits = 4
    )
  }

  ### Ratio comparing pairwise half-half clade ages.
  tree_stats_df$Tap_Xin_JCR <- round(
    tree_stats_df$Xin_Bel_TMRCAH / tree_stats_df$Tap_Xin_TMRCAH,
    digits = 4
  )

  ### -----------------------------
  ### 8c. Per-population RTH and enrichment
  ### -----------------------------

  for (pop in popNames) {
    pop_RTH    <- paste(pop, "RTH", sep = "_")
    pop_enrich <- paste(pop, "enrich", sep = "_")
    pop_n      <- as.numeric(pop_counts[pop])

    tree_stats_df[1, pop_RTH] <- round(
      getCladeNage(subtree_stats_df, pop, n = pop_n / 2) /
