...
window_size <- 500 # window size = 500 SNPs
...
saveRDS(sites, file="checkpoint.sites.win500.rds")
...
saveRDS(region(win.fn.snp)(), file="checkpoint.raw_window_regions.win500.rds")
...
saveRDS(pcdist, file="checkpoint.pcdist.filtered.win500.rds")
...
saveRDS(mds, file="checkpoint.mds.win500.rds")
...
saveRDS(win.regions, file="allele_stats.lostruct.win500.windows.rds")
...
pdf("allele_stats.lostruct.win500.pdf", height=6, width=14)
for (i in 1:nrow(sig_mds_clusters)){
  coord <- pull(sig_mds_clusters[i,1])
  direction <- pull(sig_mds_clusters[i,2])
...
dev.off()
write_tsv(cluster_genotypes, "cluster_genotypes.win500.tsv")
write_tsv(mds_info, "mds_info.win500.tsv")